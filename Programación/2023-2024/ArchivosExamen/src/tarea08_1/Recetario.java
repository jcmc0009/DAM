...
/**
 * Clase que representa un <strong>recetario</strong> para la gestión de
 * recetas.
 * <p>
 * Los objetos de esta clase permiten almacenar y gestionar una lista de objetos
 * de tipo
...
public class Recetario {
     ...
     * forma textual.
     *
     * @return cadena que representa las características del recetario de forma
     * textual
     */
    @Override
    public String toString() {
        String listado = "***********************\nLibro de Recetas\n***********************\n";
        String rutaRecetas = System.getProperty("user.dir") + "/recursos/Recetario.txt";
        if (recetas.isEmpty()) {
            listado = "El recetario está vacío";
        } else {
            for (Receta r : recetas) {
                //listado = listado + r.toString()+"\n";
                listado += "Nombre de la receta : " + r.getNombre() + "\n" + "Tipo de plato: " + r.getTipoPlato() + "\nFecha de creación: " + r.getFechaCreacion() + "\nIngredientes: " + r.getIngredientes() + "\nInstrucciones: \n" + r.getInstrucciones() + "\n*********************\n";
            }

        }
        try (FileWriter fichero = new FileWriter(rutaRecetas);//creamos el archivo
                PrintWriter escritor = new PrintWriter(fichero)) {//creamos lo que va a guardar la información en el archivo
            escritor.print(listado);//escribimos el archivod
        } catch (IOException e) {
